...
{
   Comment(""); // [MOI v1.07] xoa dong hien thi pip khi go EA, tranh chu "Hom nay: ..." bi treo lai tren chart
...
bool OpenPosition(ENUM_ORDER_TYPE type)
{
   ...
   if (InpUseDailyLossLimit && InpMaxDailyLossUSD > 0)
   {
      ...
      if (dayProfitUSD <= -MathAbs(InpMaxDailyLossUSD))
      {
         ...
         if (g_DailyLossAlertDay != dayStart)
         {
            ...
            TelegramSendMessage(StringFormat("⚠️ %s — Da lo %.2f %s hom nay, vuot nguong %.2f -- TAM DUNG vao lenh moi den sang mai",
   ...
   {
      Print("EMACrossCloneEA: khong du margin cho lenh lot=", DoubleToString(lotToUse, 2));
   ...
   if (type == ORDER_TYPE_BUY)
      ok = trade.Buy(lotToUse, _Symbol, price, sl, tp, "EMACrossCloneEA BUY");
   else
      ok = trade.Sell(lotToUse, _Symbol, price, sl, tp, "EMACrossCloneEA SELL");
   ...
   Print("EMACrossCloneEA: mo lenh ", EnumToString(type), " lot=", DoubleToString(lotToUse, 2),
   ...
   TelegramSendMessage(StringFormat("%s %s — Mo lenh %s\n📍 Gia: %.2f\n🛑 SL: %.2f\n🎯 TP: %.2f\n💰 Lot: %.2f",
                                      dirIcon, _Symbol, EnumToString(type), price, sl, tp, lotToUse));
   return true;
}
...
void OnTick()
{
   // [MOI v1.07] Dat TRUOC gate "nen moi" ben duoi co y -- can cap nhat MOI TICK (khong phai moi nen)
   // de dong lai/lo hien thi thay doi lien tuc theo gia thi truong, dung nhu yeu cau "cap nhat lien tuc".
   if (InpShowDayPips)
   {
      double dayPips = CalcDayPips();
      Comment(StringFormat("%s | Hom nay: %s%.1f pip", _Symbol, (dayPips >= 0 ? "+" : ""), dayPips));
   }
