...
int OnInit()
{
   ...
   if (InpShowDayPips) CreateDashboard(); // [MOI v1.09]
...
bool OpenPosition(ENUM_ORDER_TYPE type)
{
   ...
   TelegramSendMessage(StringFormat("%s <b>MỞ LỆNH %s</b> — %s\n\n📍 Giá vào: <b>%.2f</b>\n🛑 SL: %.2f\n🎯 TP: %.2f\n💰 Lot: %.2f%s",
                                      dirIcon, EnumToString(type), _Symbol, price, sl, tp, lotToUse, lotModeTxt));
   return true;
}
