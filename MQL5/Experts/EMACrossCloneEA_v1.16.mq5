...
//|  v1.12.                                                              |
//+------------------------------------------------------------------+
//|  v1.14 (2026-09-20): them TRAILING SL cho chan TP2 SAU KHI da ve      |
//|  breakeven -- dung y het co che da kiem chung trong TelegramSignal_EA |
//|  va EMACross_EA cua cung du an. Khac voi breakeven (chi doi SL DUNG   |
//|  1 LAN ve gia vao lenh), trailing tiep tuc KEO SL theo gia MOI TICK    |
//|  khi gia con chay them (chi keo theo huong co loi, khong bao gio lui  |
//|  lai) -- khoa them loi nhuan thay vi dung yen tai breakeven.          |
//|   - InpUseTrailingAfterTp1 (mac dinh BAT): bat/tat tinh nang.         |
//|   - InpTrailStartAtr=0.5: chi bat dau keo khi gia da di duoc it nhat  |
//|     bao nhieu x ATR tinh tu entry (tranh keo qua som).                |
//|   - InpTrailDistanceAtr=0.75: khoang cach giu SL phia sau gia hien    |
//|     tai, boi so ATR.                                                  |
//|   - InpTrailStepAtr=0.1: buoc toi thieu (boi so ATR) de cap nhat SL,  |
//|     tranh sua lenh lien tuc moi tick (spam PositionModify).           |
//|  Chi ap dung cho chan TP2 (chan TP1 da dong roi khi breakeven kich    |
//|  hoat) va CHI khi dang o dual mode + InpMoveToBreakevenOnTp1=true.    |
//|  KHONG doi logic nao khac cua v1.13.                                  |
//+------------------------------------------------------------------+
//|  v1.15 (2026-09-21): THEM dong "Diem xu huong" vao bang trang thai    |
//|  tren chart -- doc buffer moi CLONE_BUF_SCORE=5 tu indicator (chi     |
//|  co tu EMACrossClone_v1.09 tro len; ban cu hon se hien "N/A"). Diem   |
//|  0-100 la CONG THUC TU THIET KE (xem changelog v1.09 cua indicator), |
//|  KHONG PHAI cong thuc that cua "MIK SCORE" cua ban thu 3 v3.4 (chi co|
//|  .ex5, khong co source). Chi la 1 chi so THAM KHAO hien thi -- doc    |
//|  hoan toan RIENG (khong dung chung bien/ham voi buy/sell/exit signal),|
//|  KHONG anh huong bat ky logic vao/thoat lenh nao da co.               |
//+------------------------------------------------------------------+
//|  v1.16 (2026-09-22): bro bam nut Test BUY/SELL luc dang co lenh that  |
//|  mo san, bi chot an toan v1.11 chan lai (dung thiet ke, tranh chong   |
//|  lenh) -- muon co cach tu tat chot nay khi can. Them input MOI        |
//|  InpTestAllowWhilePosOpen (mac dinh FALSE, giu nguyen hanh vi an      |
//|  toan cu) -- bat len thi nut Test se LUON ep mo lenh moi, bo qua      |
//|  kiem tra "da co lenh dang mo" (co the dan den chong lenh that su     |
//|  neu bat len luc dang co lenh -- CHI dung khi bro CHU DICH muon       |
//|  test them 1 lenh nua song song).                                     |
//+------------------------------------------------------------------+
...
#define CLONE_BUF_SCORE    5 // [MOI v1.15] chi co tu indicator EMACrossClone_v1.09 tro len -- ban cu hon
                              // khong co buffer nay, CopyBuffer() se that bai (< 1), EA se hien "N/A"
...
input group "=== [MOI v1.14] Trailing SL cho chan TP2 SAU KHI da ve breakeven ==="
...
input bool InpTestAllowWhilePosOpen = false; // [MOI v1.16] true: cho phep nut Test BUY/SELL mo lenh MOI DU DA CO lenh dang mo (bo qua chot an toan chong chong lenh) -- mac dinh TAT, giu an toan cu
...
//--- [MOI v1.14] Ticket lenh dang duoc trailing (= lenh TP2 sau khi TP1 da dong va SL da ve breakeven).
//    Tach rieng khoi g_tp2Ticket vi bien do bi reset ve 0 ngay sau khi breakeven chay xong.
...
{
   string keys[] = {"Title", "DayPL", "Position", "Status", "Score"};
   int x = 10, y = 16, dy = 16;
...
int OnInit()
{
   ...
   if (InpUseDualTpMode)
   {
      ...
      {
         // [MOI v1.14] TP1 da dong tu truoc khi EA khoi dong lai -- lenh TP2 dang la "runner" (co the
         // da o breakeven hoac dang trail), tiep tuc theo doi no.
...
void UpdateDashboard()
{
   ...
   // [MOI v1.15] Doc buffer 5 (CLONE_BUF_SCORE) tu indicator -- CHI CO tu ban EMACrossClone_v1.09 tro
   // len (buffer moi, "diem chat luong" 0-100, CONG THUC TU THIET KE rieng cua tui, KHONG PHAI cong thuc
   // that cua "MIK SCORE", xem changelog v1.09 cua indicator). Doc shift=0 (nen hien tai, du dang chay
   // hay da dong deu duoc -- day chi la 1 chi so tham khao hien thi, KHONG dieu khien logic vao/thoat
   // lenh nao ca) -- neu that bai (indicator ban cu hon chua co buffer nay), hien "N/A" thay vi chan
   // dashboard hay chan giao dich.
   double scoreBuf[1];
   if (CopyBuffer(g_CloneHandle, CLONE_BUF_SCORE, 0, 1, scoreBuf) >= 1 && scoreBuf[0] != EMPTY_VALUE && MathIsValidNumber(scoreBuf[0]))
   {
      double score = scoreBuf[0];
      string scoreTxt = (score >= 70.0) ? "DEP" : (score >= 40.0 ? "TRUNG BINH" : "XAU");
      color  scoreClr = (score >= 70.0) ? clrLimeGreen : (score >= 40.0 ? clrOrange : clrTomato);
      SetLabel(DASH_PREFIX + "Score", StringFormat("Diem xu huong: %.0f/100 (%s)", score, scoreTxt), scoreClr);
   }
   else
   {
      SetLabel(DASH_PREFIX + "Score", "Diem xu huong: N/A (indicator can ban v1.09 tro len)", clrGray);
   }
}
...
// [MOI v1.14] Sau khi da ve breakeven, tiep tuc keo SL theo gia (chi keo theo huong co loi, khong bao
// gio lui lai) de giu lai them loi nhuan neu gia chay gan TP2 roi dao chieu, thay vi tra het ve dung
// breakeven. Goi moi tick (khong doi nen moi) de phan ung nhanh voi gia.
void TrailTp2Leg()
{
   ...
   double atrVal[];
   if (CopyBuffer(g_AtrHandle, 0, 1, 1, atrVal) < 1) return;
   double atr = atrVal[0];
   if (atr <= 0) return;
   ...
   if (type == POSITION_TYPE_BUY)
   {
      ...
      if (newSl > curSl + stepDist)
      {
         if (trade.PositionModify(g_trailingTicket, newSl, curTp))
            PrintFormat("EMACrossCloneEA: Trailing SL lenh #%I64u: %.2f -> %.2f", g_trailingTicket, curSl, newSl);
      }
   }
                          ...
                          const MqlTradeResult &result)
{
   ...
   if (InpUseDualTpMode && InpMoveToBreakevenOnTp1 && (g_tp1Ticket != 0 || g_tp2Ticket != 0))
   {
      ...
      if (posId == g_tp1Ticket && g_tp2Ticket != 0)
      {
         if (wasTp)
         {
            ...
            g_trailingTicket = g_tp2Ticket; // [MOI v1.14] bat dau theo doi de trail tiep tu day
         }
         else
         {
   ...
   // [MOI v1.14] Neu chinh lenh dang duoc trail vua dong (cham SL trailing hoac van chinh dich TP2),
   // dung theo doi lai.
...
void OnTick()
{
   ...
   // [MOI v1.14] Trailing SL cho chan TP2 sau breakeven -- cung dat TRUOC gate "nen moi", can chay
   // MOI TICK de phan ung kip voi gia, khong doi den nen moi moi keo SL.
   TrailTp2Leg();
...
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam)
{
   ...
   else if (sparam == DASH_PREFIX + "BtnTestBuy" || sparam == DASH_PREFIX + "BtnTestSell")
   {
      ...
      // [MOI v1.16] Cho phep tat chot nay qua InpTestAllowWhilePosOpen (mac dinh TAT = giu nguyen hanh
      // vi an toan cu) -- neu bat len, nut Test se LUON ep mo lenh moi bat ke da co lenh dang mo hay chua,
      // co the dan den chong lenh/pyramid that su, tu chiu trach nhiem neu bat.
      long existingType = -1;
      if (!InpTestAllowWhilePosOpen && GetOpenPosition(existingType) != 0)
      {
