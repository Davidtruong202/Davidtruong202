...
bool OpenPosition(ENUM_ORDER_TYPE type)
{
   ...
   string tradeComment = g_IsTestTrigger
                           ? ((type == ORDER_TYPE_BUY) ? "EMACrossCloneEA TEST BUY" : "EMACrossCloneEA TEST SELL")
                           : ((type == ORDER_TYPE_BUY) ? "EMACrossCloneEA BUY" : "EMACrossCloneEA SELL");

   bool ok;
   if (type == ORDER_TYPE_BUY)
      ok = trade.Buy(lotToUse, _Symbol, price, sl, tp, tradeComment);
   else
      ok = trade.Sell(lotToUse, _Symbol, price, sl, tp, tradeComment);
   ...
   Print("EMACrossCloneEA: ", (g_IsTestTrigger ? "[TEST] " : ""), "mo lenh ", EnumToString(type), " lot=", DoubleToString(lotToUse, 2),
   ...
   TelegramSendMessage(StringFormat("%s%s <b>MỞ LỆNH %s</b> — %s\n\n📍 Giá vào: <b>%.2f</b>\n🛑 SL: %.2f\n🎯 TP: %.2f\n💰 Lot: %.2f%s",
                                      testTag, dirIcon, EnumToString(type), _Symbol, price, sl, tp, lotToUse, lotModeTxt));
   return true;
}
