...
//+------------------------------------------------------------------+
//|  v1.09 (2026-09-21): THEM buffer moi BufScore (chi so 5, khong ve   |
//|  len chart) -- "diem chat luong" 0-100 cua xu huong hien tai, theo  |
//|  yeu cau hien thi 1 con so dep/xau kieu "MIK SCORE" len bang trang  |
//|  thai cua EA. Day la CONG THUC TU THIET KE (trung binh cong: RSI    |
//|  tho cach vung qua mua/qua ban bao xa + EMA nhanh/cham cach nhau    |
//|  bao nhieu lan ATR) -- KHONG PHAI cong thuc that cua MIK SCORE (v3.4|
//|  chi co ban .ex5, khong co source de doi chieu, xem hoi thoai da    |
//|  trao doi voi user). Buffer nay hoan toan DOC LAP, khong doc/ghi    |
//|  bat ky bien nao dung cho Buy/Sell/Exit/cac filter -- them vao      |
//|  KHONG THE lam thay doi bat ky hanh vi vao/thoat lenh nao da co.    |
//|  EA doc buffer nay qua CopyBuffer(handle, 5, ...) de hien thi.      |
//+------------------------------------------------------------------+
...
double BufSell[];
double BufExit[];
double BufScore[]; // [MOI v1.09] diem chat luong 0-100 cua xu huong hien tai -- buffer TINH TOAN (khong
                    // ve len chart), doc qua CopyBuffer(handle, 5, ...) tu EA/script ngoai, xem chi tiet
                    // cong thuc trong OnCalculate() va changelog v1.09 o dau file.
...
int OnInit()
{
   ...
   SetIndexBuffer(5, BufScore,   INDICATOR_CALCULATIONS); // [MOI v1.09] khong ve, chi de doc qua CopyBuffer
                ...
                const int &spread[])
{
   ...
   for (int k = 1; k < barsAvail; k++)
   {
      ...
      //--- [MOI v1.09] atrNow chuyen len day (vi tri cu la o doan "loc ATR" ben duoi) de dung chung cho
      //    ca BufScore (tinh ngay sau day) lan atrOK (van tinh y het nhu cu, chi doi vi tri khai bao).
      double atrNow = atrArr[k];

      //--- [MOI v1.09] BufScore: "diem chat luong" 0-100 cua xu huong hien tai, de EA doc va hien thi len
      //    dashboard (lay cam hung tu "MIK SCORE" cua MIK_EmaCross_Signal_v3.4, nhung day la CONG THUC TU
      //    THIET KE -- KHONG PHAI cong thuc that cua ho, vi ho chi cung cap ban .ex5 da compile, khong co
      //    source de doi chieu). Trung binh cong 2 thanh phan:
      //     - scoreRsi: RSI THO dang cach vung qua mua/qua ban (InpRsiOverbought/InpRsiOversold) bao xa.
      //       Huong Buy: RSI <= nguong qua mua => 100 diem (an toan); RSI cang tien ve 100 => diem cang
      //       giam ve 0. Huong Sell: doi xung voi nguong qua ban va muc 0. Day chinh la yeu to gay ra cac
      //       lenh "khong dep" (vao luc RSI da qua mua/qua ban sau) ma nguoi dung phat hien truoc do --
      //       dung chung nguong voi filter InpUseRsiZoneFilter o v1.08, du filter do co bat hay khong.
      //     - scoreTrend: khoang cach EMA nhanh/cham hien tai, tinh theo boi so ATR -- cach nhau cang xa
      //       (xu huong cang ro/manh) thi diem cang cao, toi da 100 diem tai 2.0x ATR tro len.
      //    Diem chi cap nhat KHI NEN DONG (giong Buy/Sell/Exit, KHONG phai real-time tung tick) -- dung
      //    theo triet ly "khong repaint" chung cua ca file, va la 1 CHI SO THAM KHAO doc lap, khong anh
      //    huong nguoc lai bat ky logic Buy/Sell/Exit/filter nao khac trong file.
      double scoreRsi = 50.0;
      if (priceBull)
      {
         if (!IsRsiUsable(rsiArr[k])) scoreRsi = 50.0;
         else if (rsiArr[k] <= InpRsiOverbought) scoreRsi = 100.0;
         else scoreRsi = MathMax(0.0, 100.0 * (100.0 - rsiArr[k]) / (100.0 - InpRsiOverbought));
      }
      else if (priceBear)
      {
         if (!IsRsiUsable(rsiArr[k])) scoreRsi = 50.0;
         else if (rsiArr[k] >= InpRsiOversold) scoreRsi = 100.0;
         else scoreRsi = MathMax(0.0, 100.0 * rsiArr[k] / InpRsiOversold);
      }
      double scoreTrend = 50.0;
      if (MathIsValidNumber(atrNow) && atrNow > 0)
         scoreTrend = MathMin(100.0, MathAbs(fastNow - slowNow) / atrNow / 2.0 * 100.0);
      BufScore[sIdx] = (scoreRsi + scoreTrend) / 2.0;
      ...
      //--- loc ATR (chi anh huong Buy/Sell, KHONG anh huong Exit). atrNow da duoc khai bao o tren (dung
      //    chung cho BufScore), khong khai bao lai o day nua.
