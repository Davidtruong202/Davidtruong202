                ...
                const int &spread[])
{
   ...
   for (int k = 1; k < barsAvail; k++)
   {
      ...
      if (InpUseRSIFilter)
      {
         ...
         if (!IsRsiUsable(rEma) || !IsRsiUsable(rWma)) { rsiBull = false; rsiBear = false; rsiEmptyHere = true; }
      ...
      if (crossUp)
      {
         ...
         bool buyConfirmedNow = (rsiBull && atrOK && hourOK && waitOK);
         if (buyConfirmedNow)
         {
      ...
      {
         dbgCrossDown++;
         if (rsiEmptyHere) dbgRsiEmptyDown++;

         BufExit[sIdx] = high[sIdx] + atrDistExit;

         bool sellConfirmedNow = (rsiBear && atrOK && hourOK && waitOK);
         if (sellConfirmedNow)
         {
   ...
   {
      Print("EMACrossClone: [CHAN DOAN v1.03] Tong ket tinh lai toan bo lich su da tai (", barsAvail, " nen) -- ",
            "crossUp=", dbgCrossUp, " (Buy xac nhan=", dbgBuyOK, ", RSI dang EMPTY luc do=", dbgRsiEmptyUp, ") | ",
            "crossDown=", dbgCrossDown, " (Sell xac nhan=", dbgSellOK, ", RSI dang EMPTY luc do=", dbgRsiEmptyDown, ") | ",
